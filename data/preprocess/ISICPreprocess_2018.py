...
root_dir = "../../../medical_data/ISIC_2018_Skin_Lesion"
...
# argparser.add_argument('--config_file', default='configuration.txt')
...
#
# cached_data_dir = config.data_root
...
def partition_task3_indices(k=6, i=0, test_split=0, seed=42):
    ...
    indices = np.array([idx for idx, image_id in indices], dtype=np.int)
...
def load_validation_data(task_idx, output_size=None):
    ...
    if task_idx == 1 or task_idx == 2:
        ...
        imgs = load_task3_validation_images(output_size=output_size)
        imgs = np.transpose(imgs, (0, 3, 1, 2))
        return imgs, task3_validation_image_ids
...
def load_test_data(task_idx, output_size=None):
    ...
    if task_idx == 1 or task_idx == 2:
        ...
        imgs = load_task3_test_images(output_size=output_size)
        imgs = np.transpose(imgs, (0, 3, 1, 2))
        return imgs, task3_test_image_ids


def partition_task_mix_indices(imgs, k=6, i=0, test_split=0., seed=42):
    assert isinstance(k, int) and isinstance(i, int) and 0 <= i < k
    n = len(imgs)
    n_set = int(n * (1. - test_split)) // k
...
def partition_task1_indices(k=6, i=0, test_split=0., seed=42):
    assert isinstance(k, int) and isinstance(i, int) and 0 <= i < k
    n = len(task12_image_ids)
    n_set = int(n * (1. - test_split)) // k
    indices = np.array([i for i in range(k) for _ in range(n_set)] +
                       [-1] * (n - n_set * k),
                       dtype=np.int8)
...
if __name__ == '__main__':
    (x_train, y_train), (x_valid, y_valid) = load_training_data(task_idx=1, output_size=224, num_partitions=10)
    print(x_train.shape)
    print(x_valid.shape)
    # train_indices, valid_indices = partition_task1_indices(k=10)
    # print(np.sum(np.array(train_indices,dtype=np.int)))
    # print(np.sum(np.array(valid_indices,dtype=np.int)))
    # (x_train, y_train), (x_valid, y_valid)=load_training_data(task_idx=3, output_size=224, num_partitions=10)
    # print(x_train.shape)
    # print(x_valid.shape)
    ...
    # load_validation_data(task_idx=3, output_size=224)

    # load_test_data(task_idx=1, output_size=224)
    # load_test_data(task_idx=3, output_size=224)
